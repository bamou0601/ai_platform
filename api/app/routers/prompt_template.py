...
def create_prompt(
    prompt: PromptTemplateCreate, db: Session = Depends(get_db)
):
...
def update_prompt(
    ...
    prompt = service.update_prompt(db, prompt_id, request)
...
def delete_prompt(prompt_id: int, db: Session = Depends(get_db)):
    ...
    success = service.delete_prompt(db, prompt_id)
...
# Default Prompt
@router.get("/default", response_model=PromptTemplateResponse)
...
# 有効化
@router.patch(
    "/{prompt_id}/enable", response_model=PromptTemplateResponse
)
def enable(prompt_id: int, db: Session = Depends(get_db)):
    return service.enable(db, prompt_id)


# 無効化
@router.patch(
    "/{prompt_id}/disable", response_model=PromptTemplateResponse
)
def disable(prompt_id: int, db: Session = Depends(get_db)):
    return service.disable(db, prompt_id)


# Default変更
@router.patch(
    "/{prompt_id}/default", response_model=PromptTemplateResponse
)
def set_default(prompt_id: int, db: Session = Depends(get_db)):
    return service.set_default(db, prompt_id)
