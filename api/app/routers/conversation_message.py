...
router = APIRouter(
    prefix="/conversation_messages", tags=["Conversation Messages"]
)
...
@router.get("/{id}", response_model=ConversationMessageResponse)
def get(id: int, db: Session = Depends(get_db)):
    return service.get(db, id)


@router.put("/{id}", response_model=ConversationMessageResponse)
def update(
    id: int,
